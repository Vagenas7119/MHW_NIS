...
data_TB_filtered = data_TB %>% 
  ...
  mutate(biomass_kg=ptot/1000,
         hauling_duration_hours=hauling_duration/60,
         biomass_kg_per_hour=biomass_kg/hauling_duration_hours) %>% 
  drop_na(biomass_kg_per_hour,species)
```

Some observations are extreme outliers in terms of biomass. For instance two individuals of Chimaera monstrosa that weights 1451 kg or one Dipturus oxyrinchus that weight 5400 kg. Extreme weights must be handled
```{r}
data_TB_filtered_weight_issues= data_TB_filtered %>% 
  mutate(avg_weight_kg = biomass_kg / nbtot)

# 1. Visual Inspection: Boxplots by Species
ggplot(data_TB_filtered_weight_issues, aes(x = species, y = avg_weight_kg)) +
  geom_boxplot() +
  ylab("Average Weight (kg)") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

# 2. Identify Outliers using IQR
data_TB_filtered_weight_outlier_remove <- data_TB_filtered_weight_issues %>%
  group_by(species) %>%
  mutate(
    Q1 = quantile(avg_weight_kg, 0.25, na.rm=TRUE),
    Q3 = quantile(avg_weight_kg, 0.75, na.rm=TRUE),
    IQR = Q3 - Q1,
    upper_bound = Q3 + 1.5 * IQR, # Adjust multiplier (e.g., 3 for more extreme outliers)
    lower_bound = Q1 - 1.5 * IQR,
    is_outlier = avg_weight_kg > upper_bound | avg_weight_kg < lower_bound
  ) %>%
  #Remove Outliers
  filter(avg_weight_kg >= lower_bound & 
           avg_weight_kg <= upper_bound)

#Verify removal
ggplot(data_TB_filtered_weight_outlier_remove, aes(x = species, y = avg_weight_kg)) +
  geom_boxplot() +
  ylab("Average Weight (kg)") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
...
data_TB_filtered_weight_outlier_remove_only_sp=
  data_TB_filtered_weight_outlier_remove %>% 
...
#write.csv(h3_poly_used,"h3_medits.csv")
...
data_cleaned_with_ab = data_TB_filtered_weight_outlier_remove_only_sp %>%
  select(country, unique_haul_id, h3_id, species, biomass_kg_per_hour) %>%
  group_by(unique_haul_id, h3_id, country, species) %>%
  summarise(total_biomass_kg_per_hour_per_haul = 
              sum(biomass_kg_per_hour)) %>%
  spread(key = species,
         value = total_biomass_kg_per_hour_per_haul,
         fill = 0) %>%
  #Impute zeros
  gather(key = species, value = total_biomass_kg_per_hour_per_haul, 4:ncol(.)) %>%
...
#write.csv(h3_poly_used2,"h3_medits2.csv")
```
...
data_cleaned_with_ab_rm_zaty <- data_cleaned_with_ab_st_control %>%
  ...
  filter(sum(total_biomass_kg_per_hour_per_haul) > 0) %>%
  ungroup()
...
pop_summary=data_cleaned_with_ab_rm_zaty %>% 
 ...
 #           file = "pop_summary_cleaned_MEDITS.csv")
...
data_cleaned_with_ab_rm_zaty %>% 
  group_by(unique_h3_population) %>% 
  summarise(total_biomass=sum(total_biomass_kg_per_hour_per_haul)) %>% 
  pull(total_biomass) %>% 
  range
#No
```
