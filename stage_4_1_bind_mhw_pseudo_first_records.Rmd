...
h3_map <- ggplot() +
  ...
  labs(
    title = "Unique sources for Lessepsian records",
    x = "Longitude",
    y = "Latitude"
  ) +
  theme_minimal() +
...
test_sp_dt_time_sst_mon_eff=glmmTMB(
  ...
  formula = records~scale(sources)+scale(annual_c_int)+scale(year_of_mhws)+scale(median_sst)+(1|h3_id)+(1|species),
  family = "betabinomial")#,
  #ziformula = ~1)#,
  #control = glmmTMBControl(optimizer = optim, optArgs = list(method = "BFGS")))
...
test_sp_dt_time_sst_mon_eff_sst_p=ggplot(filtered_data_before_first_record_dt_sst_meff)+
  ...
  theme_bw()+
  theme(axis.title.y = element_blank())
test_sp_dt_time_sst_mon_eff_sst_p
```
...
test_sp_dt_time_sst_mon_eff_grid
...
###monitoring effort control - no time
Test
```{r}
test_sp_dt_time_sst_mon_eff_n_time=glmmTMB(
  data = filtered_data_before_first_record_dt_sst_meff,
  formula = records~scale(sources)+scale(annual_c_int)+scale(median_sst)+(1|h3_id/year_of_mhws)+(1|species),
  family = "binomial")#,
  #control = glmmTMBControl(optimizer = optim, optArgs = list(method = "BFGS")))

summary(test_sp_dt_time_sst_mon_eff_n_time)
plot(DHARMa::simulateResiduals(test_sp_dt_time_sst_mon_eff_n_time))
performance::check_singularity(test_sp_dt_time_sst_mon_eff_n_time)
performance::check_convergence(test_sp_dt_time_sst_mon_eff_n_time)
performance::check_collinearity(test_sp_dt_time_sst_mon_eff_n_time)
test_sp_dt_time_sst_mon_eff_n_time_r2=MuMIn::r.squaredGLMM(test_sp_dt_time_sst_mon_eff_n_time)
```

Extract predictions
```{r}
test_sp_dt_time_sst_mon_eff_n_time_gg_cint=ggeffects::ggpredict(
  test_sp_dt_time_sst_mon_eff_n_time,
  terms=c("annual_c_int [0:183 by =1]"))

test_sp_dt_time_sst_mon_eff_n_time_gg_yr=ggeffects::ggpredict(
  test_sp_dt_time_sst_mon_eff_n_time,
  terms=c("year_of_mhws [1987:2022 by =1]"))

test_sp_dt_time_sst_mon_eff_n_time_gg_sst=ggeffects::ggpredict(
  test_sp_dt_time_sst_mon_eff_n_time,
  terms=c("median_sst [14:24 by = 0.5]"))

write_rds(test_sp_dt_time_sst_mon_eff_n_time_gg_cint,
          "test_sp_dt_time_sst_mon_eff_n_time_gg_cint.rds")
write_rds(test_sp_dt_time_sst_mon_eff_n_time_gg_yr,
          "test_sp_dt_time_sst_mon_eff_n_time_gg_yr.rds")
write_rds(test_sp_dt_time_sst_mon_eff_n_time_gg_sst,
          "test_sp_dt_time_sst_mon_eff_n_time_gg_sst.rds")
```

Plot
```{r}
#c_intensity
test_sp_dt_time_sst_mon_eff_n_time_cint_p=ggplot(filtered_data_before_first_record_dt_sst_meff)+
  geom_point(aes(x=annual_c_int,y=records),
             alpha=0.3,
             color="grey")+
  geom_ribbon(data=test_sp_dt_time_sst_mon_eff_n_time_gg_cint,
              aes(x=x,ymin=conf.low,ymax=conf.high),
              alpha=0.3,
              fill="#E61316")+
  geom_line(data=test_sp_dt_time_sst_mon_eff_n_time_gg_cint,
              aes(x=x,y=predicted),
            linewidth=1)+
  labs(x="Annual cumulative intensity (°C-days)",
       y="Record status",
  ...
  theme_bw()+
  theme(axis.title.y = element_blank())
test_sp_dt_time_sst_mon_eff_n_time_cint_p

#years
test_sp_dt_time_sst_mon_eff_n_time_yr_p=ggplot(filtered_data_before_first_record_dt_sst_meff)+
  geom_point(aes(x=year_of_mhws,y=records),
             alpha=0.3,
             color="grey")+
  geom_ribbon(data=test_sp_dt_time_sst_mon_eff_n_time_gg_yr,
              aes(x=x,ymin=conf.low,ymax=conf.high),
              alpha=0.3,
              fill="#E61316")+
  geom_line(data=test_sp_dt_time_sst_mon_eff_n_time_gg_yr,
              aes(x=x,y=predicted),
            linewidth=1)+
  labs(x="Year",
       y="Record status",
  ...
  theme_bw()+
  theme(axis.title.y = element_blank())
test_sp_dt_time_sst_mon_eff_n_time_yr_p

#SST
test_sp_dt_time_sst_mon_eff_n_time_sst_p=ggplot(filtered_data_before_first_record_dt_sst_meff)+
  geom_point(aes(x=median_sst,y=records),
             alpha=0.3,
             color="grey")+
  geom_ribbon(data=test_sp_dt_time_sst_mon_eff_n_time_gg_sst,
              aes(x=x,ymin=conf.low,ymax=conf.high),
              alpha=0.3,
              fill="#E61316")+
  geom_line(data=test_sp_dt_time_sst_mon_eff_n_time_gg_sst,
              aes(x=x,y=predicted),
            linewidth=1)+
  labs(x="SST (°C)",
       y="Record status",
  ...
  theme_bw()+
  theme(axis.title.y = element_blank())
test_sp_dt_time_sst_mon_eff_n_time_sst_p
```

```{r}
test_sp_dt_time_sst_mon_eff_n_time_grid=gridExtra::grid.arrange(left="Probability of record occurrence",
  test_sp_dt_time_sst_mon_eff_n_time_yr_p,
                        test_sp_dt_time_sst_mon_eff_n_time_sst_p,
                        test_sp_dt_time_sst_mon_eff_n_time_cint_p,
                        ncol = 2,
...
test_sp_dt_time_sst_mon_eff_n_time_grid
ggsave(filename ="test_sp_dt_time_sst_mon_eff_n_time_grid.png" ,plot =test_sp_dt_time_sst_mon_eff_n_time_grid ,device = "jpeg",units = "cm",width = 20,height = 15)
```
