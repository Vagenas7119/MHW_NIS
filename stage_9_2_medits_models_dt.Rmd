...
#Biomass
medits_mhw_biomass=read_rds("medits_mhw.rds") %>% 
...
medits_mhw_n=read_rds("medits_mhw_n.rds") %>% 
  left_join(annual_sst,by=c("h3_id","year_of_mhw"))
medits_mhw_n$lessepsian=as.factor(medits_mhw_n$lessepsian)
#less subset
...
test_nis=glmmTMB(data=medits_mhw_biomass_nis,
             formula = log_biomass_ratio~scale(annual_c_int)+scale(median_sst)+scale(year_of_mhw)+(1|h3_id),
...
sjPlot::plot_model(test_nis,type="eff",terms = "median_sst",show.data = T)
#predict
test_nis_n_gg=ggeffects::ggpredict(test_nis,terms="annual_c_int[0:171 by=1]")
```

all species
```{r}
test_all=glmmTMB(data=medits_mhw_biomass,
             formula = log_biomass_ratio~as.factor(lessepsian)+scale(annual_c_int)+scale(median_sst)+scale(year_of_mhw)+(1|species),
             family="gaussian",ziformula = ~1)

plot(DHARMa::simulateResiduals(test_all))
summary(test_all)
performance::check_convergence(test_all)
performance::check_singularity(test_all)
MuMIn::r.squaredGLMM(test_all)
sjPlot::plot_model(test_all,type="eff",terms = "annual_c_int",show.data = T)

sjPlot::plot_model(test_all,type="eff",terms = "median_sst",show.data = T)
...
test_nis_n=glmmTMB(data=medits_mhw_n_nis,
             formula = log_n_ratio~scale(annual_c_int)+scale(median_sst)+scale(year_of_mhw)+(1|h3_id),
             family="gaussian",ziformula = ~1,weights = weights)
...
#predict
test_nis_n_gg=ggeffects::ggpredict(test_nis_n,terms="annual_c_int[0:171 by=1]")
```

all species
```{r}
test_all_n=glmmTMB(data=medits_mhw_n,
             formula = log_n_ratio~scale(annual_c_int)*lessepsian+scale(median_sst)*lessepsian+scale(year_of_mhw)*lessepsian+(1|h3_id),
             family="gaussian",ziformula = ~1)
plot(DHARMa::simulateResiduals(test_all_n))
summary(test_all_n)
performance::check_convergence(test_all_n)
performance::check_singularity(test_all_n)
MuMIn::r.squaredGLMM(test_all_n)
sjPlot::plot_model(test_all_n,type="eff",terms = "annual_c_int",show.data = F)
sjPlot::plot_model(test_all_n,type="eff",terms = "median_sst",show.data = F)
sjPlot::plot_model(test_all_n,type="eff",terms = "year_of_mhw",show.data = F)
sjPlot::plot_model(test_all_n,type="eff",terms = "lessepsian",show.data = F)
```
...
##ggsave
ggsave(filename = "combined_ratios_p.png",plot = combined_ratios,device = "jpeg",width = 22,height = 10,units = "cm")
```
